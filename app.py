...
def certificate_page(course_id):
    uid  = session['user_id']
    cert = one(supabase.table('certificates').select('*')
               .eq('user_id', uid).eq('course_id', course_id).execute())
    if not cert:
        return redirect('/certifications')

    course = one(supabase.table('courses').select('title,category').eq('id', course_id).execute())
    user   = one(supabase.table('users').select('full_name').eq('id', uid).execute())
    uc     = one(supabase.table('user_courses').select('enrolled_at')
                 .eq('user_id', uid).eq('course_id', course_id).execute())

    return render_template('certificate.html',
        student_name = (user or {}).get('full_name') or 'Student',
        domain       = (course or {}).get('category') or (course or {}).get('title') or 'General',
        course_title = (course or {}).get('title') or '',
        cert_number  = cert.get('certificate_number') or '',
        from_date    = str((uc or {}).get('enrolled_at') or '')[:10],
        to_date      = str(cert.get('issued_at') or '')[:10],
    )
...
def api_certificates():
    uid   = session['user_id']
    certs = rows(supabase.table('certificates').select('*').eq('user_id', uid).execute())
    if not certs:
        return jsonify([])
    cids = [c['course_id'] for c in certs]
    crs  = rows(supabase.table('courses').select('id,title,category').in_('id', cids).execute())
    crs_map = {c['id']: c for c in crs}
    out = []
    for c in certs:
        co = crs_map.get(c['course_id'], {})
        out.append({
            'course_id':   c['course_id'],
            'course_title': co.get('title'),
            'category':    co.get('category'),
            'certificate_number': c.get('certificate_number'),
            'issued_at':   str(c.get('issued_at'))[:10] if c.get('issued_at') else '',
        })
    return jsonify(out)
...
def api_complete_lesson():
    ...
    if lesson:
        ...
        # NOTE: certificates are now issued only after PASSING the final
        # course test (see /api/quiz/submit and /api/self-test/pass),
        # not just from finishing all lessons.
...
# ════════════════════════════════════════════════════════
# CERTIFICATES — issued only after PASSING the final course test
# ════════════════════════════════════════════════════════
def _issue_certificate(uid, cid):
    """Issue a certificate for (uid, cid) if one doesn't already exist. Returns the cert dict."""
    existing = one(supabase.table('certificates').select('*')
                   .eq('user_id', uid).eq('course_id', cid).execute())
    if existing:
        return existing
    cert_number = 'PGI-' + datetime.utcnow().strftime('%Y') + '-' + \
...
@app.route('/api/self-test/pass', methods=['POST'])
@login_required
def api_self_test_pass():
    """Fallback: issue a certificate from the client-side self-assessment
    when a course has no real quiz questions set up yet. Requires the
    course to be 100% complete and a passing (>=60) score."""
    d     = request.get_json() or {}
    uid   = session['user_id']
    cid   = d.get('course_id')
    score = d.get('score', 0)
    if not cid or score < 60:
        return jsonify({'cert_issued': False})

    uc = one(supabase.table('user_courses').select('progress_percent')
             .eq('user_id', uid).eq('course_id', cid).execute())
    if not uc or uc.get('progress_percent') != 100:
        return jsonify({'cert_issued': False})

    cert = _issue_certificate(uid, cid)
    return jsonify({'cert_issued': bool(cert), 'course_id': cid})
...
def api_submit_quiz():
    ...
    cert_issued, cid = False, None
    if score >= 60:
        a = one(supabase.table('assignments').select('xp_reward,course_id').eq('id', aid).execute())
        if a:
            u = one(supabase.table('users').select('xp').eq('id', uid).execute())
            gained = int(a['xp_reward'] * score / 100)
            supabase.table('users').update({'xp': (u['xp'] or 0) + gained}).eq('id', uid).execute()

            cid = a['course_id']
            if cid and _issue_certificate(uid, cid):
                cert_issued = True
    ...
    return jsonify({'score': score, 'correct': correct, 'total': total,
                    'results': results, 'cert_issued': cert_issued, 'course_id': cid})
