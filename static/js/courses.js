...
function loadLesson(idx) {
  ...
  const hasVideo = !!((l.youtube_url && l.youtube_url.trim()) || (l.video_url && l.video_url.trim()));
  const hint = document.getElementById('video-lock-hint');
  if (hasVideo && !l.completed) {
...
function _renderVideo(l) {
  ...
  if (ytUrl) {
    ...
    iframe.src            = ytUrl + '?rel=0&modestbranding=1&enablejsapi=1';
  ...
  if (fileUrl) {
    ...
    video.addEventListener('ended', _unlockComplete);
...
function _watchYouTubeEnd() {
  const init = () => {
    try {
      new YT.Player('lesson-iframe', {
        events: { onStateChange: e => { if (e.data === YT.PlayerState.ENDED) _unlockComplete(); } }
      });
    } catch (e) {}
  };
...
async function submitQuiz() {
  ...
  if (res.cert_issued) {
    _showCertLink(res.course_id || quizCourseId);
  }
}

function _showCertLink(courseId) {
  const detail = document.getElementById('r-detail');
  if (!detail || !courseId) return;
  detail.insertAdjacentHTML('beforebegin',
    `<a href="/certificate/${courseId}" target="_blank" class="btn btn-primary" style="margin:10px auto;display:block;max-width:240px;text-align:center">🎓 View Your Certificate</a>`);
}
...
async function nextSelfQ() {
  if (quizCurrentQ < quizQuestions.length - 1) {
    ...
    if (score >= 60 && quizCourseId) {
      const res = await api.post('/api/self-test/pass', { course_id: quizCourseId, score });
      if (res && res.cert_issued) _showCertLink(quizCourseId);
    }
  }
}
